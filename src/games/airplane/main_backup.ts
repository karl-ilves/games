...
function createFirePlane() {
    ...
    // Fuselage (yellow with red belly)
    ...
    // Wrap in outer group to fix orientation (lookAt points +Z, but model nose is -X)
    let outerGroup = new THREE.Group();
    group.rotation.y = Math.PI / 2; // Rotate -X to +Z
    outerGroup.add(group);
    outerGroup.scale.set(CRASH_SCALE, CRASH_SCALE, CRASH_SCALE);
...
scene.fog = new THREE.Fog(0x87CEEB, 100, 25000);
...
function createAmbulance() {
    ...
    // Materials
    let carMat = new THREE.MeshLambertMaterial({ color: 0xffffff }); // White body
    let redMat = new THREE.MeshLambertMaterial({ color: 0xe74c3c }); // Red cross & stripes
    let yellowMat = new THREE.MeshLambertMaterial({ color: 0xf1c40f }); // Yellow stripes
    let blackMat = new THREE.MeshLambertMaterial({ color: 0x111111 }); // Tires & bumper
    let greyMat = new THREE.MeshLambertMaterial({ color: 0x7f8c8d }); // Grille & details
    let glassMat = new THREE.MeshLambertMaterial({ color: 0x2c3e50, transparent: true, opacity: 0.8 }); // Windows
    let silverMat = new THREE.MeshLambertMaterial({ color: 0xbdc3c7 }); // Rims
    ...
    let sirenMat = new THREE.MeshBasicMaterial({ color: 0xffffff }); // Toggled in update loop
...
for (let i = 0; i < 600; i++) {
    ...
    map1Group.add(car);
    civCars.push({ mesh: car, speed: 0.8 + Math.random() * 0.8, wheels: car.userData.wheels });
}
...
let firefighters = [];
let investigators = [];
let crashResponseState = 'none'; // 'none', 'firefighting', 'investigating'
let firetrucks = [];
...
function buildPlane(type) {
    ...
    if (type === 'airliner') {
        ...
        const bodyGeo = new THREE.BoxGeometry(1.5, 2, 10);
...
function createHumanoid(shirtColor, pantsColor, hatColor, hasExtinguisher = false) {
    let group = new THREE.Group();
...
// Hospital Logic
document.getElementById('hospital-no-btn').addEventListener('click', () => {
    document.getElementById('hospital-choice').style.display = 'none';
    document.getElementById('skip-amb-btn').style.display = 'none';
    document.getElementById('restart-btn').style.display = 'inline-block';
    
    // Show Rescue Mission UI for other passengers
    ...
    // Dispatch up to 5 ambulances teleported near the crash
    let ambCount = Math.min(5, allPatients.length);
    for (let i = 0; i < ambCount; i++) {
        let amb = createAmbulance();
        amb.position.copy(planeGroup.position);
        let aAngle = Math.random() * Math.PI * 2;
        let aDist = 50 + Math.random() * 50;
        amb.position.x += Math.cos(aAngle) * aDist;
        amb.position.z += Math.sin(aAngle) * aDist;
        amb.position.y = 1.5;
        scene.add(amb);    
        firetrucks.push({ 
            mesh: amb, 
            state: 'driving_to_crash', 
            wheels: amb.userData.wheels, 
            sirenMat: amb.userData.sirenMat, 
            isAmbulance: true,
            isPlayerAmbulance: false, // NO player patient
            timer: 0,
            capacity: 2,
            passengers: []
        });
...
document.getElementById('hospital-yes-btn').addEventListener('click', () => {
    document.getElementById('hospital-choice').style.display = 'none';
    document.getElementById('restart-btn').style.display = 'inline-block';
    ...
    for (let i = 0; i < ambCount; i++) {
        ...
        firetrucks.push({ 
            ...
            sirenMat: amb.userData.sirenMat, 
            isAmbulance: true,
            isPlayerAmbulance: (i === 0), // First ambulance gets the player
            timer: 0,
            capacity: 2, 
            passengers: []
        });
...
document.getElementById('skip-amb-btn').addEventListener('click', () => {
    document.getElementById('hospital-choice').style.display = 'none';
    document.getElementById('skip-amb-btn').style.display = 'none';
    
    // Remove ambulance if it was already called
    if (crashResponseState === 'ambulance_rescue') {
        let newFiretrucks = [];
        firetrucks.forEach(f => {
            if (f.isAmbulance) {
                scene.remove(f.mesh);
            } else {
                newFiretrucks.push(f);
            }
        });
        firetrucks = newFiretrucks;
    }
    
    const msg = document.getElementById('message');
    msg.innerText = "Skipped transport. Treatment successful.";
...
function animate() {
    ...
    if (gameState === 'playing') {
        ...
        for(let i = 0; i < debrisPieces.length; i++) {
            let p = debrisPieces[i];
            p.mesh.position.add(p.vel);
            p.mesh.rotation.x += p.rot.x;
            p.mesh.rotation.y += p.rot.y;
            p.mesh.rotation.z += p.rot.z;
            p.vel.y -= GRAVITY; // Gravity
            
            // Ground collision for debris
            if (p.mesh.position.y <= 0.5) { // Debris radius is roughly 0.5
                p.mesh.position.y = 0.5;
                if (p.vel.y < -0.5) {
                    p.vel.y *= -0.4; // Bounce
                    p.vel.x *= 0.6;  // Friction on bounce
                    p.vel.z *= 0.6;
                } else {
                    p.vel.y = 0;
                    p.vel.x *= 0.85; // Sliding friction
                    p.vel.z *= 0.85;
                }
            }
        }
...
function updatePlayer() {
    ...
    if (playerMesh.position.y <= 1.1 && playerMesh.rotation.x === 0) {
                ...
                if (t.state === 'driving_to_crash') {
                    if (!t.waypoints || t.waypoints.length === 0) {
                        ...
                        if (t.targetPatient && !t.targetPatient.rescued) {
                            ...
                            // Find next unrescued
                            let unrescued = passengerMeshes.find(p => !p.rescued);
                            if (unrescued) {
                    ...
                    if (t.waypoints && t.waypoints.length > 0) {
                        ...
                        let threshold = (t.waypoints.length === 1) ? 4000 : 400; // 63 units for final waypoint to avoid getting stuck on buildings
                        if (distSq > threshold) {
                            ...
                            let moveDir = new THREE.Vector3();
                            t.mesh.getWorldDirection(moveDir);
                            t.mesh.position.sub(moveDir.clone().multiplyScalar(8.0));
...
function updatePhysics() {
    if (playerMode) {
        ...
        if (keys['KeyW']) planeThrottle = Math.min(MAX_THROTTLE, planeThrottle + THROTTLE_RATE * 5);
        if (keys['KeyS']) planeThrottle = Math.max(0, planeThrottle - THROTTLE_RATE * 5);
    }
    ...
    if (planeGroup.position.y <= 2) {
        ...
        let fwdY = Math.max(-1, Math.min(1, forwardVector.y));
...
function triggerCrash(msg) {
    gameState = 'crashed';
    crashResponseState = 'firefighting';
    ...
    messageEl.innerText = ''; // Clear any previous messages
    document.getElementById('restart-btn').style.display = 'inline-block';
    document.getElementById('skip-amb-btn').style.display = 'inline-block';
    document.getElementById('hospital-choice').style.display = 'block';
    ...
    // Hide plane, show fireball
    planeGroup.visible = false;
    explosion = new THREE.Mesh(new THREE.SphereGeometry(5 * CRASH_SCALE, 16, 16), explosionMat);
    explosion.position.copy(planeGroup.position);
    scene.add(explosion);
    
    // Extra fires on crashed buildings
    for (let b of buildings) {
        if (b.userData.onFire) {
            ...
            for (let j = 0; j < bFireCount; j++) {
                ...
                // Spawn on the outside walls of the building
                let bW = 22; // Building width approx
                let side = Math.random();
                let offsetX = (Math.random() - 0.5) * bW;
                let offsetZ = (Math.random() - 0.5) * bW;
                if (side < 0.25) offsetX = bW;
                else if (side < 0.5) offsetX = -bW;
                else if (side < 0.75) offsetZ = bW;
                else offsetZ = -bW;
                
                let basey = b.position.y - b.userData.height / 2;
                let offsetY = Math.random() * b.userData.height;
                fire.position.set(
                ...
                fire.userData.isBuildingFire = true;
                fire.userData.basey = basey;
                fire.userData.offsetY = offsetY;
                fire.userData.height = b.userData.height;
                fire.userData.speed = (0.5 + Math.random() * 1.5) * CRASH_SCALE;
                
                scene.add(fire);
                grassFires.push(fire); // Treat as general fire so firetrucks can extinguish it
            }
        }
    }
    ...
    const colors = [0xffffff, 0xcccccc, 0xaa0000, 0xc0392b, 0x222222, 0x888888];
    let debrisCount = Math.floor(30 * CRASH_SCALE);
    for(let i = 0; i < debrisCount; i++) {
        let mat = new THREE.MeshPhongMaterial({ color: colors[Math.floor(Math.random() * colors.length)] });
        // Random sized debris
        let geo = new THREE.BoxGeometry(
            (Math.random() * 3 + 0.5) * CRASH_SCALE, 
            (Math.random() * 3 + 0.5) * CRASH_SCALE, 
            (Math.random() * 3 + 0.5) * CRASH_SCALE
        );
        let piece = new THREE.Mesh(geo, mat);
        piece.position.copy(planeGroup.position);
        ...
        // Velocity (only explosion force, do not inherit plane's forward speed so they stay at the crash site)
        let speedFactor = 1.0; 
        let vel = new THREE.Vector3(); // No forward momentum!
        vel.x += (Math.random() - 0.5) * 10 * speedFactor;
        vel.y += (Math.random() * 8 + 2) * speedFactor; // Blast upwards
        vel.z += (Math.random() - 0.5) * 10 * speedFactor;
        ...
        scene.add(piece);
        debrisPieces.push({ mesh: piece, vel: vel, rot: rotVel });
    }
    
    // Attach fires to flying debris
    let grassFireCount = Math.floor(100 * CRASH_SCALE);
    for (let i = 0; i < grassFireCount; i++) {
        let fire = new THREE.Sprite(globalFireMat.clone());
        fire.scale.set(8, 12, 1);
        
        if (debrisPieces.length > 0) {
            let targetDebris = debrisPieces[Math.floor(Math.random() * debrisPieces.length)];
            fire.userData.attachedDebris = targetDebris;
            fire.userData.offsetX = (Math.random() - 0.5) * (8 * CRASH_SCALE);
            fire.userData.offsetZ = (Math.random() - 0.5) * (8 * CRASH_SCALE);
            fire.userData.offsetY = Math.random() * 4;
        } else {
            // Fallback if no debris (shouldn't happen)
            fire.position.set(planeGroup.position.x, 0.5, planeGroup.position.z);
        }
    ...
    for (let i = 0; i < pCount; i++) {
        let pass = createHumanoid(0x3498db, 0x2c3e50, null, false); // Blue shirt
        pass.position.copy(planeGroup.position);
        pass.position.x += (Math.random() - 0.5) * 60 * CRASH_SCALE;
        pass.position.z += (Math.random() - 0.5) * 60 * CRASH_SCALE;
        pass.position.y = 0.5;
        pass.rotation.x = -Math.PI / 2; // Lie down injured
        pass.rotation.z = Math.random() * Math.PI * 2;
        scene.add(pass);
        passengerMeshes.push({ mesh: pass, rescued: false });
    }
    ...
    for (let i = 0; i < ffCount; i++) {
        ...
        // Spawn them ~100 units away from crash
        let angle = Math.random() * Math.PI * 2;
        ffGroup.position.set(
            planeGroup.position.x + Math.cos(angle) * 100,
            0,
            planeGroup.position.z + Math.sin(angle) * 100
        );
        
        scene.add(ffGroup);
        firefighters.push({
            mesh: ffGroup,
            leftArm: ffGroup.userData.leftArm,
            rightArm: ffGroup.userData.rightArm,
            leftLeg: ffGroup.userData.leftLeg,
            rightLeg: ffGroup.userData.rightLeg,
            waterJet: ffGroup.userData.waterJet,
            legTime: Math.random() * 5
        });
    }
    
    // Spawn Firetrucks
    let ftCount = Math.max(2, Math.floor(12 * CRASH_SCALE));
    for (let i = 0; i < ftCount; i++) {
        let truckGroup = new THREE.Group();
        let truckMat = new THREE.MeshLambertMaterial({ color: 0xc0392b });
        let whiteMat = new THREE.MeshLambertMaterial({ color: 0xecf0f1 });
        let windowMat = new THREE.MeshLambertMaterial({ color: 0x111111 });
        let greyMat = new THREE.MeshLambertMaterial({ color: 0x7f8c8d });
        
        // Cabin (Front) - Positive Z is forward
        let cabinGeo = new THREE.BoxGeometry(5.8, 4.5, 5);
        let cabin = new THREE.Mesh(cabinGeo, truckMat);
        cabin.position.set(0, 3, 5);
        truckGroup.add(cabin);
        
        // Cabin Windows
        let windGeo = new THREE.BoxGeometry(6, 1.5, 2.5);
        let windows = new THREE.Mesh(windGeo, windowMat);
        windows.position.set(0, 3.8, 5.5);
        truckGroup.add(windows);
        
        // Rear Body
        let rearGeo = new THREE.BoxGeometry(6.2, 4.0, 10);
        let rearBody = new THREE.Mesh(rearGeo, truckMat);
        rearBody.position.set(0, 2.75, -2.5);
        truckGroup.add(rearBody);
        
        // White stripe along the side
        let stripeGeo = new THREE.BoxGeometry(6.3, 0.6, 15);
        let stripe = new THREE.Mesh(stripeGeo, whiteMat);
        stripe.position.set(0, 2.5, 0);
        truckGroup.add(stripe);
        
        // Ladder on top
        let ladderGeo = new THREE.BoxGeometry(1.5, 0.5, 12);
        let ladder = new THREE.Mesh(ladderGeo, greyMat);
        ladder.position.set(0, 5, -2);
        truckGroup.add(ladder);
        
        // Light bar
        let lightMat = new THREE.MeshBasicMaterial({ color: 0x3498db });
        let light = new THREE.Mesh(new THREE.BoxGeometry(4, 0.5, 0.8), lightMat);
        light.position.set(0, 5.5, 5); // on top of cabin
        truckGroup.add(light);
        
        // 6 Wheels
        let truckWheels = [];
        let wheelMat = new THREE.MeshLambertMaterial({ color: 0x111111 });
        let wheelGeo = new THREE.CylinderGeometry(1.2, 1.2, 1.5, 16);
        wheelGeo.rotateZ(Math.PI/2);
        let positions = [
            [-3.5, 1.2, 5], [3.5, 1.2, 5],     // front axle
            [-3.5, 1.2, -2], [3.5, 1.2, -2],   // middle axle
            [-3.5, 1.2, -5], [3.5, 1.2, -5]    // rear axle
        ];
        for (let p of positions) {
            let w = new THREE.Mesh(wheelGeo, wheelMat);
            w.position.set(p[0], p[1], p[2]);
        ...
        // Water Cannon (on front roof)
        let waterMat = new THREE.MeshBasicMaterial({ color: 0x3498db, transparent: true, opacity: 0.7 });
        let waterGeo = new THREE.CylinderGeometry(0.5, 0.5, 1, 8); 
        let cannon = new THREE.Mesh(waterGeo, waterMat);
        cannon.position.set(0, 6.2, 6); // moved slightly forward and up
        cannon.rotation.x = Math.PI / 2;
        cannon.visible = false;
        truckGroup.add(cannon);
        
        let tAngle = (Math.random() * Math.PI * 2) + (i * Math.PI/2.5);
        let tDist = 50 + Math.random() * 50; // Teleport near crash
        truckGroup.position.set(
            planeGroup.position.x + Math.cos(tAngle) * tDist,
            0,
            planeGroup.position.z + Math.sin(tAngle) * tDist
        );
    ...
    // Spawn Firefighting Planes
    let fpCount = Math.max(1, Math.floor(1.5 * CRASH_SCALE));
    for (let i = 0; i < fpCount; i++) {
        let fp = createFirePlane();
        
        let tAngle = (Math.random() * Math.PI * 2);
        let tDist = 2000 + Math.random() * 1000;
        ...
        // Point towards crash
        fp.mesh.lookAt(planeGroup.position.x, fp.mesh.position.y, planeGroup.position.z);
        
        scene.add(fp.mesh);
        firePlanes.push({
            mesh: fp.mesh,
            props: fp.props,
            targetPos: new THREE.Vector3(planeGroup.position.x, fp.mesh.position.y, planeGroup.position.z),
            dropped: false,
            speed: 5 + Math.random() * 2,
            lifeAfterDrop: 300 // frames before despawn
        });
    }
    ...
    playerMesh.visible = true;
    playerVelocity.copy(planeVelocity);
    playerVelocity.y += 10; // Blast up
    playerVelocity.x += (Math.random() - 0.5) * 10;
    playerVelocity.z += (Math.random() - 0.5) * 10;
...
function updateCamera() {
    ...
    if (playerMode) {
        ...
        const camX = targetMesh.position.x + Math.sin(cameraAngleX) * Math.cos(cameraAngleY) * camDist;
        const camY = Math.max(0.5, targetMesh.position.y + Math.sin(cameraAngleY) * camDist);
        const camZ = targetMesh.position.z + Math.cos(cameraAngleX) * Math.cos(cameraAngleY) * camDist;
    ...
    if (gameState === 'crashed' && explosion) {
        const time = Date.now() * 0.001;
        let targetPosition = explosion.position.clone().add(new THREE.Vector3(Math.cos(time)*100, 50, Math.sin(time)*100));
        camera.position.lerp(targetPosition, 0.1);
        camera.lookAt(explosion.position);
        camera.up.set(0, 1, 0);
        return;
    }
...
let antiIceSystem = false;
let iceAmount = 0; // 0 to 1 (ice accumulation)

let weatherState = {
    windTimer: 0,
    windNextAction: 300, // 10 secs
    windDirection: 1, // 1 for down, -1 for up
    snowTimer: 0,
    snowTimerActive: false, // track if we are in a timed snow event
    iceGracePeriod: 900, // 30 seconds at 30fps
    rainSystem: null,
    snowSystem: null,
    cloudsGroup: null,
    thunderNextFlash: 0
...
const btnSetWeather = document.getElementById('btn-set-weather');
const weatherSelect = document.getElementById('weather-select');

function toggleAntiIce() {
    antiIceSystem = !antiIceSystem;
    if (antiIceSystem) {
...
btnAntiIce.addEventListener('click', toggleAntiIce);
...
btnSetWeather.addEventListener('click', () => {
    let type = weatherSelect.value;
    if (type === 'random') {
        const types = ['sun', 'clouds', 'rain', 'thunder', 'wind', 'snow'];
        type = types[Math.floor(Math.random() * types.length)];
        weatherSelect.value = type;
    }
    setWeather(type);
});
...
function createParticles(type) {
    ...
    for(let i=0; i<count*3; i+=3) {
        posArray[i] = planeGroup.position.x + (Math.random() - 0.5) * 1000;
        posArray[i+1] = Math.random() * 500;
        posArray[i+2] = planeGroup.position.z + (Math.random() - 0.5) * 1000;
    }
    geo.setAttribute('position', new THREE.BufferAttribute(posArray, 3));
    
    let mat;
    if (type === 'rain') {
        mat = new THREE.PointsMaterial({ color: 0xaaaaaa, size: 2, transparent: true, opacity: 0.6 });
    } else {
        // Snow
        mat = new THREE.PointsMaterial({ color: 0xffffff, size: 3, transparent: true, opacity: 0.8 });
    }
    const system = new THREE.Points(geo, mat);
    return system;
}

function updateParticles(system, type, deltaY) {
    if (!system) return;
    const positions = system.geometry.attributes.position.array;
    const px = planeGroup.position.x;
    const pz = planeGroup.position.z;
    for(let i=1; i<positions.length; i+=3) {
        positions[i] -= deltaY; // move down
        // drift
        if (type === 'snow') {
            positions[i-1] += (Math.random() - 0.5) * 1.5;
            positions[i+1] += (Math.random() - 0.5) * 1.5;
        } else {
            positions[i-1] += 0.5; // wind drift for rain
        }
        
        // Wrap vertically
        if (positions[i] < 0) {
            positions[i] = 500 + Math.random() * 200;
        }
        // Wrap horizontally around the plane
        if (positions[i-1] < px - 500) positions[i-1] += 1000;
        if (positions[i-1] > px + 500) positions[i-1] -= 1000;
        if (positions[i+1] < pz - 500) positions[i+1] += 1000;
        if (positions[i+1] > pz + 500) positions[i+1] -= 1000;
    }
...
function createClouds() {
    if (weatherState.cloudsGroup) scene.remove(weatherState.cloudsGroup);
    weatherState.cloudsGroup = new THREE.Group();
    const cloudGeo = new THREE.SphereGeometry(150, 8, 8);
    ...
    for (let i = 0; i < 30; i++) {
        ...
        weatherState.cloudsGroup.add(cloud);
    }
    scene.add(weatherState.cloudsGroup);
}
...
function setWeather(type) {
    ...
    // Reset effects
    if (weatherState.rainSystem) { scene.remove(weatherState.rainSystem); weatherState.rainSystem = null; }
    if (weatherState.snowSystem) { scene.remove(weatherState.snowSystem); weatherState.snowSystem = null; }
    if (weatherState.cloudsGroup) { scene.remove(weatherState.cloudsGroup); weatherState.cloudsGroup = null; }
    document.getElementById('weather-alert').style.display = 'none';
    scene.background = new THREE.Color(0x87CEEB);
    scene.fog = new THREE.Fog(0x87CEEB, 200, 4000);
    
    weatherState.windTimer = 0;
    weatherState.snowTimerActive = false;
    ...
    switch(type) {
        ...
        case 'clouds':
            createClouds();
            scene.fog.density = 0.001; // slight fog
            break;
        case 'rain':
            createClouds();
            weatherState.rainSystem = createParticles('rain');
        ...
        case 'thunder':
            ...
            weatherState.thunderNextFlash = 60 + Math.random() * 300;
            break;
        case 'wind':
            weatherState.windTimer = 120 * 30; // 2 minutes
            weatherState.windNextAction = 300; // 10 secs
            weatherState.windDirection = 1; // start by pushing down
            document.getElementById('weather-alert-title').innerText = 'Strong Wind - 2 mins';
            document.getElementById('weather-alert').style.display = 'block';
            break;
        case 'snow':
            weatherState.snowSystem = createParticles('snow');
            scene.add(weatherState.snowSystem);
            scene.background = new THREE.Color(0xdddddd);
            scene.fog = new THREE.FogExp2(0xdddddd, 0.002);
            
            weatherState.snowTimer = 300 * 30; // 5 minutes
            weatherState.snowTimerActive = true;
            weatherState.iceGracePeriod = 1800; // 30 sec reset
            document.getElementById('weather-alert-title').innerText = 'Snowstorm - 5 mins';
            document.getElementById('weather-alert').style.display = 'block';
            break;
    }
}
...
function updateWeather() {
    ...
    // Update UI lock state (can't change weather during timed events)
    const ws = document.getElementById('weather-select');
    const bw = document.getElementById('btn-set-weather');
    const wm = document.getElementById('weather-menu');
    if (ws && bw) {
        let locked = (weatherState.windTimer > 0) || weatherState.snowTimerActive;
        ws.disabled = locked;
        bw.disabled = locked;
    ...
    if (weatherState.rainSystem) {
        updateParticles(weatherState.rainSystem, 'rain', 10 + Math.random() * 5);
    }
    // Animate Snow
    if (weatherState.snowSystem) {
        updateParticles(weatherState.snowSystem, 'snow', 2 + Math.random() * 2);
    }
    
    // Thunderstorm flashes and turbulence
    if (currentWeather === 'thunder') {
        weatherState.thunderNextFlash--;
        if (weatherState.thunderNextFlash <= 0) {
            scene.background = new THREE.Color(0xffffff); // flash
            setTimeout(() => {
                if (currentWeather === 'thunder') scene.background = new THREE.Color(0x222233);
            }, 100);
            weatherState.thunderNextFlash = 120 + Math.random() * 400;
        }
        // Heavy turbulence
        planeGroup.rotateZ((Math.random() - 0.5) * 0.02);
        planeGroup.rotateX((Math.random() - 0.5) * 0.01);
    }
    else if (currentWeather === 'clouds' || currentWeather === 'rain') {
        // Light turbulence
        planeGroup.rotateZ((Math.random() - 0.5) * 0.005);
    }
    
    // Strong Wind Logic
    if (weatherState.windTimer > 0) {
        weatherState.windTimer--;
        
        let secs = Math.ceil(weatherState.windTimer / 30);
        let m = Math.floor(secs / 60);
        let s = secs % 60;
        document.getElementById('weather-alert-time').innerText = `Time remaining: ${m}:${s < 10 ? '0'+s : s}`;
        
        weatherState.windNextAction--;
        if (weatherState.windNextAction <= 0) {
            // Push plane up or down 500 feet
            let offset = 500 * weatherState.windDirection;
            if (planeGroup.position.y - offset > 100) {
                planeGroup.position.y -= offset;
            }
            weatherState.windDirection *= -1; // reverse direction for next time
            weatherState.windNextAction = 300; // 10 secs
        }
        
        if (weatherState.windTimer <= 0) {
            setWeather('sun'); // end wind
        }
    }
    
    // Snow Timer Logic
    if (weatherState.snowTimerActive && weatherState.snowTimer > 0) {
        weatherState.snowTimer--;
        
        let secs = Math.ceil(weatherState.snowTimer / 30);
        let m = Math.floor(secs / 60);
        let s = secs % 60;
        document.getElementById('weather-alert-time').innerText = `Time remaining: ${m}:${s < 10 ? '0'+s : s}`;
        
        if (weatherState.snowTimer <= 0) {
            setWeather('sun');
        }
    }
    
    // Ice Accumulation Physics
    if (currentWeather === 'snow' && !antiIceSystem) {
        if (weatherState.iceGracePeriod > 0) {
            weatherState.iceGracePeriod--;
        } else {
            iceAmount += 0.0005; // builds up
            if (iceAmount > 1) iceAmount = 1;
        }
    } else if (antiIceSystem) {
        iceAmount -= 0.002; // melts faster than builds
        if (iceAmount < 0) iceAmount = 0;
    }
    
    // Apply ice physics: Pitch down force
    if (iceAmount > 0) {
        let icePitchForce = iceAmount * 0.005; // max 0.005 pitch down per frame
        planeGroup.rotateX(icePitchForce); // Positive X is pitch down in this engine
        
        // Visual ice effect on plane
        planeGroup.traverse(child => {
            if (child.isMesh && child.material) {
                let mats = Array.isArray(child.material) ? child.material : [child.material];
                mats.forEach(m => {
                    if (m.origEmissive === undefined) {
                        m.origEmissive = m.emissive ? m.emissive.clone() : new THREE.Color(0x000000);
                    }
                    let iceCol = new THREE.Color(0xffffff).multiplyScalar(iceAmount * 0.6);
                    m.emissive = m.origEmissive.clone().add(iceCol);
                });
            }
        });
    } else {
        // Reset emissive when ice is 0
        planeGroup.traverse(child => {
            if (child.isMesh && child.material) {
                let mats = Array.isArray(child.material) ? child.material : [child.material];
                mats.forEach(m => {
                    if (m.origEmissive !== undefined) {
                        m.emissive = m.origEmissive.clone();
                    }
                });
            }
        });

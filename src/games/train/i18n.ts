...
export const I18N = {
    et: {
        ...
        depotDesc: 'Vali oma rong või metroo ning ava uusi sõidukeid Rongiraha (+50€ jaamaga) või Playbuxidega (Playbux = 5x mänguraha)!',
        depotMoneyLabel: '🪙 Rongiraha:',
        depotYardLabel: '💎 Playbux:',
        depotStartDriving: '🚂 ALUSTA SÕITU',
        depotFree: 'TASUTA',
        depotOwned: 'OMATUD ✓',
        depotSelected: 'VALITUD ⭐',
        selectBtn: 'VALI',
        topSpeed: 'Tippkiirus:',
        accel: 'Kiirendus:',
        capacity: 'Mahutavus:',
        ...
        buyYard: (price: number) => `💎 OSTA (${price} pbx)`,
        boughtSuccessMoney: (name: string, price: number) => `🎉 Ostsid edukalt sõiduki "${name}" Rongiraha eest (${price} €)!`,
        boughtSuccessYard: (name: string, price: number) => `🎉 Ostsid edukalt sõiduki "${name}" Playbuxi eest (${price} pbx)!`,
        notEnoughMoney: (price: number, cur: number) => `Sul pole piisavalt Rongiraha! Vajad ${price} € (praegu ${cur} €). Sõida ja tee jaamapeatustega +50€!`,
        notEnoughYards: (price: number, cur: number) => `Sul pole piisavalt Playbuxi! Vajad ${price} pbx (praegu ${cur} pbx).`,
        speedUnit: 'KM / H',
    ...
    en: {
        ...
        depotDesc: 'Select your locomotive or metro train and unlock vehicles with Train Money (+50€ per stop) or Playbux (Playbux = 5x train price)!',
        ...
        buyYard: (price: number) => `💎 BUY (${price} pbx)`,
        boughtSuccessMoney: (name: string, price: number) => `🎉 Successfully purchased "${name}" with Train Money (${price} €)!`,
        boughtSuccessYard: (name: string, price: number) => `🎉 Successfully purchased "${name}" with Playbux (${price} pbx)!`,
        notEnoughMoney: (price: number, cur: number) => `Not enough Train Money! You need ${price} € (you have ${cur} €).`,
        notEnoughYards: (price: number, cur: number) => `Not enough Playbux! You need ${price} pbx (you have ${cur} pbx).`,
        speedUnit: 'KM / H',
